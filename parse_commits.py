...
rfile = open('results/' + sys.argv[1] + '.csv', 'w')
wr = csv.writer(rfile, quoting=csv.QUOTE_ALL)
wr.writerow(["code_quantity", "commit_quality", "message", "author", "repo"] )
...
def commit_quality(message):
    words = Scrabble.process_commit_message(message)
    s = Scrabble()
...
def parse_commit(commit: github.Commit.Commit):
    ...
    for file in commit.files:
        ...
        if not patch:
            continue
        patches = list(whatthepatch.parse_patch(patch))
        if len(patches) < 1:
            continue
        diff = patches[0].changes
        for change in diff:
            if change.new != None:
                stats = code_quantity(change.line)
                quantity.update(stats)
    quality = commit_quality(commit.commit.message)
...
try:
    for idx, line in enumerate(tqdm(lines)):
        ...
        if line == "!!python/object:github.Commit.Commit\n":
            if (idx - currentLines[0]) > 0:
                ...
                commit : github.Commit.Commit = yaml.load(commit, Loader=yaml.Loader)
                ...
                wr.writerow([
                    ...
                    commit.author.login if commit.author else '',
                    commit.html_url
                ])
                rfile.flush()
                counter += 1
    ...
    logging.error("Shutting down because of {} {}".format(type(e).__name__, e))
    import traceback
    traceback.print_exc()
finally:
    rfile.close()
